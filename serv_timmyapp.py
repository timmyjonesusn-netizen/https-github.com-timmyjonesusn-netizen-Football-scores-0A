# serv_timmyapp.py — TimmyApp (restored last-session style, no f-strings)
import os, random
...
# ----- Style & UI bits (same look as last session) -----
def css_global():
    return (
        "<style>"
        ":root{--bg1:#1a0731;--bg2:#3b0b5e;--ink:#f7e9ff;--glow:#ff4fd8;--blue:#6bb0ff}"
...
def bubbles_js():
    # Purple bubbles with occasional blue + rare white pops (like last session)
    return (
        "<script>(function(){"
...
def navbar(active):
    def link(href, label, key, extra=""):
        ...
        if key == active:
            return "<span class='btn " + extra + "' style='background:rgba(255,79,216,.2)'>" + label + "</span>"
        return base
    # Events removed; Police button is transparent blue
    return (
        "<div class='nav'>"
        + link("/home","Home","home")
...
# ----- Weather helper (same as last session) -----
...
def create_app():
    ...
    def police():
        ...
        return page_shell("police", "Police Corner", inner, "Transparent blue nav button")
    ...
    def music():
        ...
        return page_shell("music", "Music", inner, "Five playlists ready")

    @app.route("/weather")
    def weather():
        wt = weather_now_text()
        inner = "<div class='card'><h3>Ragland Weather (°F)</h3><p>" + wt + "</p></div>"
        return page_shell("weather", "Weather", inner, "Live pull via Open-Meteo")
    ...
    def learn():
        ...
        inner = (
            ...
            "<p>" + r["q"] + "</p>"
            "<details><summary>Show choices</summary><ul>"
            + "".join(["<li>" + c + "</li>" for c in r["choices"]]) +
            "</ul></details>"
            "<details><summary>Reveal answer</summary><p><strong>" + r["answer"] + "</strong></p></details>"
            "</div>"
            "</div>"
        )
